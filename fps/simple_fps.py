...
player = FirstPersonController(
    model='cube', z=-10, color=color.orange, origin_y=-.5, speed=15)

player.collider = BoxCollider(player, Vec3(0, 1, 0), Vec3(1, 2, 1))

gun = Entity(model='cube', parent=camera, position=(.5, -.25, .25),
             scale=(.3, .2, 1), origin_z=-.5, color=color.red, on_cooldown=False)
gun.muzzle_flash = Entity(parent=gun, z=1, world_scale=.6,
                          model='sphere', color=color.blue, enabled=False)
...
def shoot():
    if not gun.on_cooldown:
        ...
        invoke(gun.muzzle_flash.disable, delay=.04)
        invoke(setattr, gun, 'on_cooldown', False, delay=.09)
...
class Enemy(Entity):
    ...
    def hp(self, value):
        ...
        if value <= 0:
            ...
            destroy(self)
            if (destroy):
                enemies = [Enemy(x=x*4) for x in range(2)]
            return
...
# Enemy()
enemies = [Enemy(x=x*4) for x in range(9)]


def pause_input(key):
    if key == 'tab' or key == '0':    # press tab to toggle edit/play mode
